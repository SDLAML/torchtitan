...
from torchtitan.components.legacy_dataloader import ParallelAwareDataloader
...
@dataclass
class DatasetConfig:
    """Describes one HF dataset: where it lives and how to read a sample.

    Previously exported from ``torchtitan.hf_datasets``; upstream emptied that
    module when the grain data stack replaced it, so it lives here now.
    """
...
def _flat_collate(
    ...
    return {
        ...
        "positions": positions,
        # Kept on CPU as a plain int; the trainer sums it across DP ranks
...
class HuggingFaceTextDataLoader(ParallelAwareDataloader):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Upstream sizes batches in tokens; this loader still packs fixed-length
        # sequences, so recover the rectangle it needs and flatten at collate.
        seq_len = max_context_length
        if num_tokens_per_batch % seq_len != 0:
            raise ValueError(
                f"num_tokens_per_microbatch_per_dp_rank ({num_tokens_per_batch}) "
                f"must be a multiple of max_context_length ({seq_len}); this "
                "loader packs whole sequences."
            )
