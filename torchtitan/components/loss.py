...
class MoEAuxLoss(BaseLoss):
    ...
    NOTE: upstream #3864 added ``models/common/aux_loss.AuxLoss`` plus
    ``register_aux_loss_zero_hook``, so an upstream equivalent now EXISTS -- this
    docstring previously said it did not. Adopting it would delete this class,
    but it logs a mean over layers rather than our per-layer
    ``moe_load_balance_loss/L-*`` (see optimizers/container.py), and upstream's
    own docstring notes its metric over-counts 2x under FullAC. Deferred as a
    separate change rather than folded into the upstream merge.
