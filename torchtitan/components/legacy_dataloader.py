...
"""torchdata-backed dataloader, vendored forward from the 0.4.0 base.

Upstream replaced this with the grain-based ``components/data`` package. That
package cannot express dynamic data mixing: ``DatasetMixConfig`` is frozen and
calls ``grain.IterDataset.mix(children, weights)`` once at build time, so there
is no runtime ``set_weights`` for ``DataMixScheduler`` to drive, no per-dataset
document/token counters, and an exhausted child stops the mix instead of being
dropped from it. Upstream's own ``TODO(data-token-weighted-mix)`` marks that
work as not yet done.

So the torchdata stack stays, but as a peer of the grain stack rather than a
replacement: ``ParallelAwareDataloader`` derives from upstream's
``BaseDataLoader``, so both satisfy one protocol and upstream models keep
working unchanged.
"""
...
__all__ = ["BaseDataLoader", "DataloaderExhaustedError", "ParallelAwareDataloader"]
...
class ParallelAwareDataloader(StatefulDataLoader, BaseDataLoader):
    ...
    class Config(BaseDataLoader.Config):
        dataset: str = ""
        """Dataset name. Upstream's BaseDataLoader.Config dropped these two
        fields when it moved to grain; the mixed text loader still keys off them."""

        dataset_path: str | None = None
        """Filesystem or hub path for the dataset."""
