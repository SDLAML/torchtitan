...
class Trainer(torch.distributed.checkpoint.stateful.Stateful, Configurable):
    ...
    def train_step(self, data_iterator: Iterator[dict[str, Any]]):
        ...
        # getattr, not attribute access: this is a fork-only convenience, and
        # upstream's trainer tests build a fake config that carries no
        # `training.steps`. Hard-accessing it made those tests fail on our
        # addition rather than on anything they were testing.
        should_log = self.metrics_processor.should_log(self.step) or (
            self.step == getattr(self.config.training, "steps", None)
        )
        ...
        with sl.log_trace_span("optim"):
            ...
            # getattr for the same reason as `should_log` above: this DiSCO-only
            # block must not make upstream's trainer tests fail on a fake config
            # that carries no `metrics` / `training.steps`. With a real config the
            # behaviour is unchanged; without one, norm logging is simply off.
            log_norm_freq = getattr(
                getattr(self.config, "metrics", None), "log_norm_freq", 0
            )
